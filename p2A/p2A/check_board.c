...
int valid_board(int **board, int size) {
    ...
    // check for invalid elements within each row

    for(int row = 0; row < size; row++){
	for(int col = 0; col < size; col++){
	    // tracker is the index of the number we are checking
	    for(int tracker = col + 1; tracker < size; tracker++){
		if((*(*(board + row) + col) > size) || ((*(*(board + row) + col) != 0)
			&& (*(*(board + row) + col) == *(*(board + row) + tracker)))){
		   return 0;
		}
	    }
	}	
    ...
    // check for invalid elements within each column

    for(int row = 0; row < size; row++){
        for(int col = 0; col < size; col++){
            for(int tracker = col + 1; tracker < size; tracker++){
                if((*(*(board + col) + row) > size) || ((*(*(board + col) + row) != 0)
                        && (*(*(board + col) + row) == *(*(board + tracker) + row)))){
                   return 0;
                }
            }
        }
