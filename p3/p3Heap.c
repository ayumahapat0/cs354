...
void* balloc(int size) {     
    ...
    // closestFitptr -> size_status = closestFit - (closestFit - totalSize)
...
int bfree(void *ptr) {    
    ...
    if(((int)ptr - (int)heap_start) % 8 != 0){
        return -1; 
    }

    if(((int)ptr < heap_start) ||((int) ptr > (int)(heap_start) + alloc_size) ){
        return -1;
    }
...
int coalesce() {
    //TODO: Your code goes in here.
    blockHeader *start = heap_start; 
    blockHeader *freeBlocks = start; 
    int counter = 0;
    int coalesceHappen = 0; 
    
    while(start -> size_status != 1){
        if((start -> size_status) & 1){
            start += ((start -> size_status & ~3)/sizeof(blockHeader));
        }else{
            freeBlocks = start; 
            while(freeBlocks -> size_status & 1 != 1){
                counter++;
                freeBlocks += ((freeBlocks -> size_status & ~3)/sizeof(blockHeader));
            }
            if(counter < 2){
               start += ((start -> size_status & ~3)/sizeof(blockHeader)); 
            }
                ...
                for(int i = 0; i < (counter - 1); i++){
                    int increase = (start + ((start -> size_status & ~3)/sizeof(blockHeader))) -> size_status;
                    start -> size_status += increase; 
                }
                start += ((start -> size_status & ~3)/sizeof(blockHeader));
                counter = 0;
                coalesceHappen = 1;
